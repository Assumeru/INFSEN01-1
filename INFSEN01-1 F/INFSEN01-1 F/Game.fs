...
let changeTile x y tile state =
    {state with map = Functions.Map.SetTile(state.map, x, y, tile)}
...
let fireballDir dir x y state =
    ...
    if (state.player.mp >= 3) then
        ...
        if(tileExists x y state) then
            ...
            match tile with
            | 'w' -> "Your fireball destroyed a wall", changeTile x y 'o' state
...
let parseCommand x state =
    if(state.running) then
        ...
        | "help" -> helpCommandsNormalState|> String.concat "\n", state
...
let runFrame state =
    hitPlayer state state.monsters
